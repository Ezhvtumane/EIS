...
select ILL_HISTORY,ID_CASE, YEAR(DATE_END) data,DATE_END, sumib, kolIB,kolsly4,1 flag 
into #t_eis 
from 
(
select  ILL_HISTORY,ID_CASE,DATE_END, SUM(cost) sumib, COUNT(*) kolIB, SUM(kol) kolsly4  from(
select ILL_HISTORY,t3.ID_CASE, t3.DATE_END, sum(service_cost) cost, COUNT(*) kol from OPENquery(EISS, 'select * from case_services_accounts ') t
left join 
(
...
group by ILL_HISTORY,t3.ID_CASE,t3.DATE_END
) t
group by ILL_HISTORY, ID_CASE,DATE_END
) t

--Отказы стационар
insert into #t_eis 
select  ILLH,ID_CASE, YEAR(data) data, data, sum(cost), 1 kolib ,COUNT(*) kolsly4,-1 flag  from
(
select  
...
and (t2.ID_CASE_CAST<>1	or t22.ID_CASE_CAST<>1)																															
) t
group by ILLH,ID_CASE,data
