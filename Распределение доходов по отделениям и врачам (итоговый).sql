...
/*
Во временную таблицу #t_prices записываем цену услуги и ее код. Берем самую последнюю (по PLH_Code) цену.
*/
/*
...
/*
Данные по ИБ и АК из ЕИСа. 
Пишем во временну таблоицу #t_eis
*/
...
/*
Во временную таблицу #t_services записываем услуги (сейчас к ним дописывается цена, можно вынести куда-либо еще), вытягиваем из поля FLDctxtNameDocument номер ИБ,АК и тд. 
У койко-дней меняем CRFromFLD на CRFromD, так как они выполняются из ПД, и номер ИБ тянем из istortable по DOCUNIDу.
*/
...
where DEPT_NAME not like 'Поликлиническое' --and ill_history='7994'
