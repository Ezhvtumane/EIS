/*
Запрос делает выборку по историям болезней и амбулаторным услугам (читай приемы врачей по ОМС) из Единой Информационной Системы ОМС СПб за определенный период дат.
Так же учитываются отказы за этот же промежуток времени.
После этого ищем эти же истории и услуги в базе КМИСа и считаем сколько было полученно на каждую историю и услугу из Фонда по ОМС.
Таким образом смотрим сколько каждое отделение заработало (или ушло в "-") по ОМС профилю финансирования.
*/
...
select ctxtOtd collate Cyrillic_General_CI_AS,EXCECUROR_NAME collate Cyrillic_General_CI_AS,ANALIS_NAME collate Cyrillic_General_CI_AS, 
				name collate Cyrillic_General_CI_AS,DATE ,1 isskol,sum(cost)  ,
				Parent_Doc_ID collate Cyrillic_General_CI_AS, otdclin
			from 
				(select 
